...
static void grid_btn_event_cb(lv_event_t * e)
{
    if(lv_event_get_code(e) == LV_EVENT_CLICKED) {
        ...
        square_to_text((uint8_t)square_id, &status_text[lvgl_ui_selected_color == 1U ? 4 : 5]);
        if(grid_status_label != NULL) {
            lv_label_set_text(grid_status_label, status_text);
        }
    }
}
...
static void create_grid_page(uint8_t color_id)
{
    ...
    lv_obj_set_width(grid_status_label, SCREEN_W);
    lv_obj_set_pos(grid_status_label, 0, STATUS_LABEL_Y);
    lv_obj_set_style_text_align(grid_status_label, LV_TEXT_ALIGN_CENTER, LV_STATE_DEFAULT);
    lv_obj_set_style_text_color(grid_status_label, color_border(color_id), LV_STATE_DEFAULT);
    lv_obj_set_style_text_font(grid_status_label, &lv_font_montserrat_32, LV_STATE_DEFAULT);
    lv_label_set_text(grid_status_label, color_id == 1U ? "RED" : "BLUE");
