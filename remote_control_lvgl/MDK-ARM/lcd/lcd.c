...
volatile unsigned int g_lcd_wait_timeout = 0;
volatile unsigned char g_lcd_wait_last_reg = 0;
volatile unsigned char g_lcd_wait_last_value = 0;
volatile unsigned char LCD_PLL_Reg00_Before=0;
volatile unsigned char LCD_PLL_Reg00_After=0;
volatile unsigned char LCD_PLL_Reg00_Last=0;
volatile unsigned int LCD_PLL_Wait_Count=0;
volatile unsigned char LCD_PLL_Timeout=0;
volatile unsigned int LCD_GPIOE_MODER_Snap=0;
volatile unsigned int LCD_GPIOE_ODR_Snap=0;
volatile unsigned int LCD_GPIOE_IDR_Snap=0;
volatile unsigned int LCD_GPIOD_MODER_Snap=0;
volatile unsigned int LCD_GPIOD_ODR_Snap=0;
volatile unsigned int LCD_GPIOD_IDR_Snap=0;
volatile unsigned int LCD_Debug_Stage=0;
volatile unsigned char LCD_PWM_Reg85=0;
volatile unsigned char LCD_PWM_Reg86=0;
volatile unsigned char LCD_PWM_Reg88=0;
volatile unsigned char LCD_PWM_Reg89=0;
volatile unsigned char LCD_PWM_Reg8A=0;
volatile unsigned char LCD_PWM_Reg8B=0;
volatile unsigned char LCD_PWM_Reg8C=0;
volatile unsigned char LCD_StatusRead_Debug=0;
volatile unsigned short LCD_RawFSMC_Debug=0;
volatile unsigned char LCD_Reg084_Debug=0;
volatile unsigned char LCD_SDRAM_E0_Debug=0;
volatile unsigned char LCD_SDRAM_E4_Before_Debug=0;
volatile unsigned char LCD_SDRAM_E4_After_Debug=0;
volatile unsigned short LCD_SDRAM_itv_Debug=0;
volatile unsigned char LCD_SDRAM_Status_Debug=0;
volatile unsigned char LCD_SDRAM_Try_Index_Debug=0xff;
volatile unsigned char LCD_SDRAM_Try_E0_Set_Debug=0;
volatile unsigned char LCD_SDRAM_Try_E1_Set_Debug=0;
volatile unsigned char LCD_SDRAM_Try_E0_Read_Debug=0;
volatile unsigned char LCD_SDRAM_Try_Timing_Debug=0;
volatile unsigned char LCD_SDRAM_Ready_Debug=0;
volatile unsigned char LCD_PWM_Reg8D=0;
volatile unsigned char LCD_PWM_Reg8E=0;
volatile unsigned char LCD_PWM_Reg8F=0;
volatile unsigned char LCD_Reg10_Debug=0;
volatile unsigned char LCD_Reg12_Debug=0;
volatile unsigned char LCD_Reg13_Debug=0;
volatile unsigned char LCD_RegF3_Debug=0;
volatile unsigned char LCD_RegF4_Debug=0;
volatile unsigned char LCD_SDRAM_Status_Last=0;
volatile unsigned int LCD_SDRAM_Wait_Count=0;
volatile unsigned char LCD_SDRAM_Timeout=0;
volatile unsigned char LCD_Init_Stage=0;
volatile unsigned char LCD_Skip_Init_Font=1;
volatile unsigned int LCD_Check_Timeout_Count=0;
volatile unsigned char LCD_Check_Last_ID=0;
volatile unsigned char LCD_SFI_DMA_Timeout=0;
volatile unsigned int LCD_SFI_DMA_Wait_Count=0;
 ...
 void LCD_PLL_Initial(void) 
{
	...
	// 23-138MHZ,27-162MHZ,30-180MHZ,32-192MHZ;
	 unsigned int MCLK_M = 18; // lowered further for SDRAM stability
	...
	unsigned int CCLK_M = 18; 	 // lowered further for LCD controller stability
	...
	LCD_PLL_Wait_Count = 0;
LCD_PLL_Timeout = 0;
while ((LCD_Reg_Read(0x00) & 0x80) == 0)
{
    delay_us(100);
    LCD_PLL_Wait_Count++;
    if(LCD_PLL_Wait_Count > 5000U)
    {
        LCD_PLL_Timeout = 1;
        break;
    }
...
void Check_Mem_WR_FIFO_not_Full(void)
{
    ...
    {
        if(--timeout == 0U) { LCD_Check_Timeout_Count++; break; }
    }
}
...
void Check_SDRAM_Ready(void)
{
    ...
    unsigned int timeout = 100000U;
    LCD_Check_Last_ID = 6;
    LCD_SDRAM_Wait_Count = 0;
    LCD_SDRAM_Timeout = 0;
    do
    {
        temp=LCD_StatusRead();
        LCD_SDRAM_Status_Last = temp;
        LCD_SDRAM_Wait_Count++;
        if(--timeout == 0U) {
            LCD_SDRAM_Timeout = 1;
            LCD_Check_Timeout_Count++;
            break;
        }
    }while( (temp&0x04) == 0x00 );
    if((temp & 0x04) != 0U) LCD_SDRAM_Ready_Debug = 1;
}
...
void Check_Busy_SFI_DMA(void)
{
    unsigned int timeout = 100000U;
    LCD_CmdWrite(0xB6);
    LCD_SFI_DMA_Wait_Count = 0;
    LCD_SFI_DMA_Timeout = 0;
    while((LCD_DataRead()&0x01)==0x01)
    {
        LCD_SFI_DMA_Wait_Count++;
        if(--timeout == 0U) {
            LCD_SFI_DMA_Timeout = 1;
            LCD_Check_Timeout_Count++;
            break;
        }
    }
