...
volatile uint32_t lv_refr_debug_stage = 0;
volatile uint32_t lv_refr_debug_inv_p = 0;
volatile int32_t lv_refr_debug_i = 0;
volatile int32_t lv_refr_debug_last_i = 0;
volatile int32_t lv_refr_debug_x1 = 0;
volatile int32_t lv_refr_debug_y1 = 0;
volatile int32_t lv_refr_debug_x2 = 0;
volatile int32_t lv_refr_debug_y2 = 0;
volatile uint32_t lv_refr_debug_px_num = 0;
volatile uint32_t lv_refr_debug_draw_buf_flushing = 0;
volatile uint32_t lv_refr_debug_draw_buf_last_area = 0;
volatile uint32_t lv_refr_debug_draw_buf_last_part = 0;
volatile uint32_t lv_refr_debug_area_w = 0;
volatile uint32_t lv_refr_debug_area_h = 0;
volatile uint32_t lv_refr_debug_row = 0;
volatile uint32_t lv_refr_debug_max_row = 0;
volatile uint32_t lv_refr_debug_disp = 0;
volatile uint32_t lv_refr_debug_drv = 0;
volatile uint32_t lv_refr_debug_draw_ctx = 0;
volatile uint32_t lv_refr_debug_draw_buf = 0;
volatile uint32_t lv_refr_debug_buf = 0;
volatile uint32_t lv_refr_debug_buf_area = 0;
volatile uint32_t lv_refr_debug_obj = 0;
volatile uint32_t lv_refr_debug_top_obj = 0;
volatile uint32_t lv_refr_debug_flush_cb = 0;
volatile uint32_t lv_refr_debug_color_p = 0;
...
void _lv_disp_refr_timer(lv_timer_t * tmr)
{
    ...
    lv_refr_debug_stage = 100;

    if(tmr) {
        disp_refr = tmr->user_data;
        lv_refr_debug_stage = 110;
        ...
        lv_refr_debug_stage = 111;
    }

    /*Refresh the screen's layout if required*/
    lv_refr_debug_stage = 120;
    lv_obj_update_layout(disp_refr->act_scr);
    if(disp_refr->prev_scr) lv_obj_update_layout(disp_refr->prev_scr);

    lv_obj_update_layout(disp_refr->top_layer);
    lv_obj_update_layout(disp_refr->sys_layer);
    lv_refr_debug_stage = 121;

    /*Do nothing if there is no active screen*/
    if(disp_refr->act_scr == NULL) {
        lv_refr_debug_stage = 122;
        disp_refr->inv_p = 0;
    ...
    lv_refr_debug_stage = 130;
    lv_refr_join_area();
    lv_refr_debug_stage = 131;
    refr_sync_areas();
    lv_refr_debug_stage = 132;
    refr_invalid_areas();
    lv_refr_debug_stage = 133;

    /*If refresh happened ...*/
    if(disp_refr->inv_p != 0) {
        lv_refr_debug_stage = 140;
        ...
        /*Clean up*/
        lv_refr_debug_stage = 150;
        lv_memset_00(disp_refr->inv_areas, sizeof(disp_refr->inv_areas));
        lv_refr_debug_stage = 151;
        lv_memset_00(disp_refr->inv_area_joined, sizeof(disp_refr->inv_area_joined));
        lv_refr_debug_stage = 152;
        disp_refr->inv_p = 0;

        lv_refr_debug_stage = 153;
        elaps = lv_tick_elaps(start);

        /*Call monitor cb if present*/
        if(disp_refr->driver->monitor_cb) {
            lv_refr_debug_stage = 154;
            disp_refr->driver->monitor_cb(disp_refr->driver, elaps, px_num);
        }
    }

    lv_refr_debug_stage = 160;
    lv_mem_buf_free_all();
    lv_refr_debug_stage = 161;
    _lv_font_clean_up_fmt_txt();
    lv_refr_debug_stage = 162;

#if LV_DRAW_COMPLEX
    _lv_draw_mask_cleanup();
#endif
    lv_refr_debug_stage = 163;
...
static void refr_invalid_areas(void)
{
    lv_refr_debug_stage = 200;
    px_num = 0;
    lv_refr_debug_px_num = 0;
    lv_refr_debug_inv_p = disp_refr->inv_p;

    if(disp_refr->inv_p == 0) {
        lv_refr_debug_stage = 201;
        return;
    }
    ...
    for(i = 0; i < disp_refr->inv_p; i++) {
        ...
        if(disp_refr->inv_area_joined[i] == 0) {
            lv_refr_debug_stage = 205;
            lv_refr_debug_x1 = disp_refr->inv_areas[i].x1;
            lv_refr_debug_y1 = disp_refr->inv_areas[i].y1;
            lv_refr_debug_x2 = disp_refr->inv_areas[i].x2;
            lv_refr_debug_y2 = disp_refr->inv_areas[i].y2;

            if(i == last_i) disp_refr->driver->draw_buf->last_area = 1;
            disp_refr->driver->draw_buf->last_part = 0;
            lv_refr_debug_draw_buf_last_area = disp_refr->driver->draw_buf->last_area;
            lv_refr_debug_draw_buf_last_part = disp_refr->driver->draw_buf->last_part;
            lv_refr_debug_stage = 206;
            refr_area(&disp_refr->inv_areas[i]);
            lv_refr_debug_stage = 207;

            px_num += lv_area_get_size(&disp_refr->inv_areas[i]);
            lv_refr_debug_px_num = px_num;
        }
    }

    lv_refr_debug_stage = 208;
...
static void refr_area(const lv_area_t * area_p)
{
    lv_refr_debug_stage = 220;
    lv_refr_debug_x1 = area_p->x1;
    lv_refr_debug_y1 = area_p->y1;
    lv_refr_debug_x2 = area_p->x2;
    lv_refr_debug_y2 = area_p->y2;
    lv_draw_ctx_t * draw_ctx = disp_refr->driver->draw_ctx;
    lv_refr_debug_draw_ctx = (uint32_t)draw_ctx;
    draw_ctx->buf = disp_refr->driver->draw_buf->buf_act;
    lv_refr_debug_buf = (uint32_t)draw_ctx->buf;
    ...
    lv_refr_debug_area_w = (uint32_t)w;
    lv_refr_debug_area_h = (uint32_t)h;
    lv_refr_debug_max_row = (uint32_t)max_row;
    if(max_row <= 0) {
        lv_refr_debug_stage = 231;
        return;
    }
    ...
    for(row = area_p->y1; row + max_row - 1 <= y2; row += max_row) {
        lv_refr_debug_stage = 232;
        lv_refr_debug_row = (uint32_t)row;
        ...
        lv_refr_debug_buf = (uint32_t)draw_ctx->buf;
        lv_refr_debug_buf_area = (uint32_t)draw_ctx->buf_area;
        lv_refr_debug_x1 = sub_area.x1;
        lv_refr_debug_y1 = sub_area.y1;
        lv_refr_debug_x2 = sub_area.x2;
        lv_refr_debug_y2 = sub_area.y2;
        lv_refr_debug_draw_buf_last_part = disp_refr->driver->draw_buf->last_part;
        lv_refr_debug_stage = 233;
        refr_area_part(draw_ctx);
        lv_refr_debug_stage = 234;
    }

    /*If the last y coordinates are not handled yet ...*/
    if(y2 != row_last) {
        lv_refr_debug_stage = 235;
        /*Calc. the next y coordinates of draw_buf*/
        sub_area.x1 = area_p->x1;
        sub_area.x2 = area_p->x2;
        sub_area.y1 = row;
        ...
        lv_refr_debug_y2 = sub_area.y2;
        lv_refr_debug_draw_buf_last_part = disp_refr->driver->draw_buf->last_part;
        lv_refr_debug_stage = 236;
        refr_area_part(draw_ctx);
        lv_refr_debug_stage = 237;
    }
}

static void refr_area_part(lv_draw_ctx_t * draw_ctx)
{
    lv_refr_debug_stage = 250;
    lv_refr_debug_draw_ctx = (uint32_t)draw_ctx;
    lv_disp_draw_buf_t * draw_buf = lv_disp_get_draw_buf(disp_refr);
    lv_refr_debug_draw_buf = (uint32_t)draw_buf;

    if(draw_ctx->init_buf) {
        lv_refr_debug_stage = 251;
    ...
    if((draw_buf->buf1 && !draw_buf->buf2) ||
       (draw_buf->buf1 && draw_buf->buf2 && full_sized)) {
        lv_refr_debug_stage = 252;
        while(draw_buf->flushing) {
            lv_refr_debug_draw_buf_flushing = draw_buf->flushing;
    ...
    if(disp_refr->draw_prev_over_act) {
        ...
        if(disp_refr->prev_scr) {
            if(top_prev_scr == NULL) top_prev_scr = disp_refr->prev_scr;
            lv_refr_debug_top_obj = (uint32_t)top_prev_scr;
            lv_refr_debug_stage = 273;
            refr_obj_and_children(draw_ctx, top_prev_scr);
        }

        if(top_act_scr == NULL) top_act_scr = disp_refr->act_scr;
        lv_refr_debug_top_obj = (uint32_t)top_act_scr;
        lv_refr_debug_stage = 274;
        refr_obj_and_children(draw_ctx, top_act_scr);
    }

    /*Also refresh top and sys layer unconditionally*/
    lv_refr_debug_stage = 275;
    refr_obj_and_children(draw_ctx, lv_disp_get_layer_top(disp_refr));
    lv_refr_debug_stage = 276;
    refr_obj_and_children(draw_ctx, lv_disp_get_layer_sys(disp_refr));

    lv_refr_debug_stage = 280;
    draw_buf_flush(disp_refr);
    lv_refr_debug_stage = 281;
}
...
static void refr_obj_and_children(lv_draw_ctx_t * draw_ctx, lv_obj_t * top_obj)
{
    lv_refr_debug_stage = 300;
    lv_refr_debug_top_obj = (uint32_t)top_obj;
    /*Normally always will be a top_obj (at least the screen)
     *but in special cases (e.g. if the screen has alpha) it won't.
     *In this case use the screen directly*/
    if(top_obj == NULL) top_obj = lv_disp_get_scr_act(disp_refr);
    if(top_obj == NULL) {
        lv_refr_debug_stage = 301;
        return;  /*Shouldn't happen*/
    }
    lv_refr_debug_top_obj = (uint32_t)top_obj;

    /*Refresh the top object and its children*/
    lv_refr_debug_stage = 302;
    ...
    while(parent != NULL) {
        lv_refr_debug_stage = 303;
        lv_refr_debug_obj = (uint32_t)parent;
        bool go = false;
        uint32_t i;
        ...
        for(i = 0; i < child_cnt; i++) {
            ...
            if(!go) {
                ...
                /*Refresh the objects*/
                lv_refr_debug_stage = 304;
                lv_refr_debug_obj = (uint32_t)child;
                refr_obj(draw_ctx, child);
            }
        }

        /*Call the post draw draw function of the parents of the to object*/
        lv_refr_debug_stage = 305;
...
void refr_obj(lv_draw_ctx_t * draw_ctx, lv_obj_t * obj)
{
    ...
    if(layer_type == LV_LAYER_TYPE_NONE) {
        ...
        while(layer_ctx->area_act.y1 <= layer_area_full.y2) {
            if(flags & LV_DRAW_LAYER_FLAG_CAN_SUBDIVIDE) {
                lv_refr_debug_stage = 330;
                layer_alpha_test(obj, draw_ctx, layer_ctx, flags);
            }

            lv_refr_debug_stage = 331;
            lv_obj_redraw(draw_ctx, obj);
...
static void draw_buf_flush(lv_disp_t * disp)
{
    lv_refr_debug_stage = 400;
    lv_refr_debug_disp = (uint32_t)disp;
    lv_disp_draw_buf_t * draw_buf = lv_disp_get_draw_buf(disp_refr);
    lv_refr_debug_draw_buf = (uint32_t)draw_buf;
    ...
    lv_refr_debug_stage = 407;
}

static void call_flush_cb(lv_disp_drv_t * drv, const lv_area_t * area, lv_color_t * color_p)
{
    lv_refr_debug_stage = 420;
    lv_refr_debug_drv = (uint32_t)drv;
    lv_refr_debug_flush_cb = (uint32_t)drv->flush_cb;
    lv_refr_debug_color_p = (uint32_t)color_p;
    lv_refr_debug_x1 = area->x1;
    lv_refr_debug_y1 = area->y1;
    lv_refr_debug_x2 = area->x2;
    lv_refr_debug_y2 = area->y2;
    ...
    lv_refr_debug_stage = 421;
    drv->flush_cb(drv, &offset_area, color_p);
    lv_refr_debug_stage = 422;
}
