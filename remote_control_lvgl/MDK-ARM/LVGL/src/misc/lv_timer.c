...
volatile uint32_t lv_timer_debug_timer = 0;
volatile uint32_t lv_timer_debug_cb = 0;
volatile uint32_t lv_timer_debug_period = 0;
volatile uint32_t lv_timer_debug_last_run = 0;
volatile uint32_t lv_timer_debug_user_data = 0;
volatile uint32_t lv_timer_debug_repeat_count = 0;
volatile uint32_t lv_timer_debug_bad_cb_count = 0;
volatile uint32_t lv_timer_debug_bad_cb = 0;
volatile uint32_t lv_timer_debug_stage = 0;
volatile uint32_t lv_timer_debug_next = 0;
volatile uint32_t lv_timer_debug_already_running = 0;
volatile uint32_t lv_timer_debug_timer_created = 0;
volatile uint32_t lv_timer_debug_timer_deleted = 0;
...
{
    lv_timer_debug_stage = 100;
    TIMER_TRACE("begin");

    /*Avoid concurrent running of the timer handler*/
    static bool already_running = false;
    if(already_running) {
        lv_timer_debug_stage = 101;
        lv_timer_debug_already_running++;
...
static bool lv_timer_exec(lv_timer_t * timer)
{
    lv_timer_debug_stage = 200;
    lv_timer_debug_timer = (uint32_t)timer;
    if(timer->paused) {
        lv_timer_debug_stage = 201;
        return false;
    }

    bool exec = false;
    if(lv_timer_time_remaining(timer) == 0) {
        lv_timer_debug_stage = 210;
        ...
        timer->last_run = lv_tick_get();
        lv_timer_debug_timer = (uint32_t)timer;
        lv_timer_debug_cb = (uint32_t)timer->timer_cb;
        lv_timer_debug_period = timer->period;
        lv_timer_debug_last_run = timer->last_run;
        lv_timer_debug_user_data = (uint32_t)timer->user_data;
        lv_timer_debug_repeat_count = (uint32_t)timer->repeat_count;

        TIMER_TRACE("calling timer callback: %p", *((void **)&timer->timer_cb));
        if(timer->timer_cb && original_repeat_count != 0) {
            uint32_t cb = (uint32_t)timer->timer_cb;
            if((cb < 0x08000001U) || (cb > 0x08100000U) || ((cb & 1U) == 0U)) {
                lv_timer_debug_stage = 211;
                lv_timer_debug_bad_cb = cb;
                lv_timer_debug_bad_cb_count++;
                timer->paused = 1;
                return false;
            }
            lv_timer_debug_stage = 212;
            timer->timer_cb(timer);
            lv_timer_debug_stage = 213;
        }
        TIMER_TRACE("timer callback %p finished", *((void **)&timer->timer_cb));
        lv_timer_debug_stage = 214;
